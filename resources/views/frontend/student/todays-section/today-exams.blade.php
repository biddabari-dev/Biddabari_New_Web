                        ...
                        <div class="courses-details-accordion">
                            <ul class="accordion">
                                @if(!empty($courseExams))
                                    @forelse($courseExams as $courseSectionContent)
                                    ...
                                    @empty
                                        <li class="accordion-item">
                                            <a class="accordion-title" href="javascript:void(0)">
                                                <p>আজকে কোনো কোর্স এক্সাম নেই |</p>
                                            </a>
                                        </li>
                                    @endforelse
                                @endif
                            ...
                            <ul class="accordion">
                                @if(!empty($batchExams))
                                    @forelse($batchExams as $batchExam)
                                        <li class="accordion-item">
                                            @if($batchExam->content_type == 'pdf')
                                                <a href="javascript:void(0)" data-content-id="{{ $batchExam->id }}" class="w-100 show-pdfx accordion-title f-s-26">
                                                    <span class="f-s-26"><i class="ri-file-text-line"></i> {{ $batchExam->title }}</span>
                                                </a>
                                            @endif
                                                @if($batchExam->content_type == 'note')
                                                    <a href="javascript:void(0)" class="w-100 get-text-data accordion-title f-s-26" data-content-id="{{ $batchExam->id }}">
                                                        <span class="f-s-26"><i class="ri-file-text-line"></i> {{ $batchExam->title }}</span>
                                                    </a>
                                                @endif
                                            @if($batchExam->content_type == 'exam')
                                                <a href="javascript:void(0)" class="w-100 get-text-data accordion-title f-s-26" data-content-id="{{ $batchExam->id }}">
                                                    <span class="f-s-26"><i class="ri-file-text-line"></i> {{ $batchExam->title }}</span>
                                                </a>
                                            @endif
                                            @if($batchExam->content_type == 'written_exam')
                                                <a href="javascript:void(0)" class="w-100 get-text-data accordion-title f-s-26" data-content-id="{{ $batchExam->id }}">
                                                    <span class="f-s-26"><i class="ri-file-text-line"></i> {{ $batchExam->title }}</span>
                                                </a>
                                            @endif
                                    ...
                                    @empty
                                        <li class="accordion-item">
                                            <a class="accordion-title" href="javascript:void(0)">
                                                <p>আজকে কোনো ব্যাচ এক্সাম নেই |</p>
                                            </a>
                                        </li>
                                    @endforelse
                                @endif
...
@section('js')
    ...
    {{--    note--}}
    <script>
        $(document).on('click', '.get-text-datax', function () {
    ...
    <script>
        $(document).on('click', '.show-pdf', function () {
            ...
            if (status == true)
            {
                ...
                $.ajax({
                    ...
                    url: "https://biddabaribucket.s3.ap-southeast-1.amazonaws.com/student/show-pdf/"+sectionContentId,
                    method: "GET",
                    success: function (data) {
                        var pdflink = '';
        ...
        $(document).on('click', '.show-pdfx', function () {
            event.preventDefault();
            var sectionContentId = $(this).attr('data-content-id');
            $.ajax({
